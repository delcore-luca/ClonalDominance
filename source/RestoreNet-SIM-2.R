...
res_alls20 <- parLapply(cl = cl,
                        ...
                        fun = function(s20){
                          ...
                          resPath <- paste("./results-sim2-s2", s20, "/", sep = "")
...
relErr.all <- sapply(1:length(s20_all), function(j){
  ...
  GLS.file <- read.table(paste(currResPath, paste0("/results-sim2-s2",s20,"/sim2-s2",s20,"_GLSResults.tsv"), sep = ""), sep = "\t", header = F)
...
pdf(file = paste(currDir_time, "/allBoxplots_sim2.pdf", sep = ""), width = 10, height = 3)
...
save.image(paste(currDir_time, "/sim2_alls20.RData", sep = ""))
gc()
